...
public class StringChopperBean {
	...
	public String getResult() {
		...
		if (this.targetString != null && this.maxLength != 0) {
			if (this.targetString.length() > this.maxLength) {
				Matcher pMatcher = this.punctuationPattern
						.matcher(this.targetString);
				Matcher regPMatcher = pMatcher
						.region(Math
								.round((float) this.maxLength
										* (100 - this.punctuationSearchIntervalPercentage)
										/ 100), this.maxLength);
				int index = -1;
				String itemMatched = null;
				while (regPMatcher.find()) {
					itemMatched = regPMatcher
							.group(PUNCTUATION_REGEX_TARGET_GROUP);
			...
			throw new IllegalStateException(
					"the targetString and maxLength "
	 ...
	 */
	public void setPunctuationSearchIntervalPercentage(
			int punctuationSearchIntervalPercentage) {
		if (punctuationSearchIntervalPercentage <= 100
				&& punctuationSearchIntervalPercentage >= 0) {
